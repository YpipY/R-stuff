...
model = lmer(CHI_MLU ~ VISIT * Diagnosis +  (1+VISIT|SUBJ), data = training, REML = FALSE)
summary(model)



powerSim(model,simr::fixed('VISIT:DiagnosisTD',method='t'),nsim=1000) #Testing the power for interaction effect
powerSim(model,simr::fixed('VISIT',method='t'),nsim=1000) #Testing the power for one of my main effects
powerSim(model,simr::fixed('DiagnosisTD',method='t'),nsim=1000) #Testing the power for my other main effect.
...
Testing the power for my linear mixed effect model: CHI_MLU ~ VISIT * Diagnosis + (1 + VISIT | SUBJ)

Using 1000 simulations, the observed power for the estimate of the main effect 'VISIT' (0.10296) is calculated to 97% with confidence intervals ranging from 96.10-98.21.
The observed power for for the estimate of the main effect 'DiagnosisTD'  (-0.18994) (note that this effect is not significant at p > 0.05 and a std. error of 0.17322) is calculated to 18% with a confidence interval of 15.85 - 20.73. 
The observed power for the estimate of the interaction effect 'VISIT:DiagnosisTD' (0.24711) in my model is calculated to 100%, with confidence intervals ranging from 99.63-100.0. The power analysis indicates that the estimates are highly reliable and that our study is overpowered. 

The interaction effect can be interpreted as Visit increases by  1, the slope for CHI_MLU increases with 0.24711 for children diagnosed TD. After 5 visits (VISIT 1 being an intercept) this results in a difference of (5*0.24711) 1.23555 CHI_MLU across diagnosis.. 
```{r}
...
VISIT = (training$VISIT - mean(training$VISIT)) / sd(training$VISIT)
