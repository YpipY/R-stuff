...
library(pacman)
p_load(lme4,simr,fakeR,tidyverse)
```

### Exercise 1
```{r, include=FALSE}
# Setting WD
setwd("C:/Users/slmoni/Documents/Uni/Experimental Methods III/R-stuff/Assignment2/part4")
# Loading data
...
powerSim(simpleModel,fixed('DiagnosisTD:VISIT',method='t'),nsim=10)
...
powerSim(simpleModel,simr::fixed('DiagnosisTD:VISIT',method='t'),nsim=1000)
# Powersim for the diagnosis
powerSim(simpleModel,simr::fixed('DiagnosisTD', method='t'),nsim=1000)
# Powersim for the Visit
powerSim(simpleModel,simr::fixed('VISIT', method='t'),nsim=1000)
```
...
A power simulations was run on the interaction effect of diagnosis and visits. 1000 simulations where run with p-value optained via a Students t-test and 95% confidence interval. The simulation found 100 % power.
Power simulations were also run for diagnosis and visit in the same way. The power of the diagnosis predictor was 22.8% and the power the visit predictor was 95.3%. However, it does not make much sense to look at the power of the individual predictors since we this is a interaction effect.
...
# Runing the simulations for the power curve for the minimum interaction effect
pc<-powerCurve(simpleModelLow, simr::fixed('DiagnosisTD:VISIT',method='t'),along = 'SUBJ')
pc
plot(pc)
pc4<-powerCurve(simpleModelLow, simr::fixed('DiagnosisTD:VISIT',method='t'),along = 'SUBJ', breaks= c(20:30))
pc4
plot(pc4)

# Runing the simulations for the power curve for the minimum effect of diagnosis

# Creating a model with more partisipants
#simData<-select(train,SUBJ,VISIT, Diagnosis, CHI_MLU)
#simData<- simulate_dataset_ts(simData, digits=5, n=6228, cluster='VISIT', time.variable='CHI_MLU', ignore=c('Diagnosis','SUBJ'))
#?simulate_dataset
#simpleModelLow=lmer(CHI_MLU  ~  Diagnosis * VISIT  +  (1  +  VISIT  |  SUBJ), data= simData,REML=F)
#summary(simpleModelLow)
#simpleModelLow@beta[2]<--0.1866996

#pc5<-powerCurve(simpleModelLow, simr::fixed('DiagnosisTD',method='t'),along = 'SUBJ',breaks=c(100,200,300,400,500))
#pc5
#plot(pc5)

# Runing the simulations for the power curve for the minimum effect of visit
trainD<-train
trainD$CHI_MLU<-(train$CHI_MLU-mean(train$CHI_MLU))/sd(train$CHI_MLU)
simpleModelD=lmer(CHI_MLU  ~  Diagnosis * VISIT  +  (1  +  VISIT  |  SUBJ), data= trainD,REML=F)
summary(simpleModelD)

simpleModelD@beta[3]<-0.2

pc6<-powerCurve(simpleModelD, simr::fixed('VISIT',method='t'),along = 'SUBJ')
pc6
plot(pc6)
pc7<-powerCurve(simpleModelD, simr::fixed('VISIT',method='t'),along = 'SUBJ', breaks= c(15:25))
pc7
plot(pc7)
```
...
An simulation of the power was run varying number of participants. 
For the interaction between diagnosis and visit, it was found that at and above 23 participant the power of the effect would reliably be above 80%.
For visits and it was found that at and above 19 participant the power of the effect would reliably be above 80%.
