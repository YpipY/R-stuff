...
doubletrain <- merge(train, train, by = "SUBJ")

diagmodel = lmer(CHI_MLU.x ~ VISIT.x * Diagnosis.x + (1 + VISIT.x | SUBJ), doubletrain , REML = F)
fixef(diagmodel)["Diagnosis.x"] <- -0.15

powerCurveDiag = powerCurve(diagmodel, simr::fixed("Diagnosis.x", method = "t"),along="X.x", nsim=100)
plot(powerCurveDiag)
powerCurveDiag
#NOT DONE
