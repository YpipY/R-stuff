...
```{r}
setwd("C:/Users/slmoni/Documents/Uni/R-stuff/Assignment1&2_LanguageASD")
```
...
?subset
colnames(ADS1)[7] <- "ADOS1"
ADSGOOD<-join(ADS,ADS1,by = c("SUBJ", "VISIT"))

ADSHalf<-merge(ADS, ADS1, by.x="ADOS", by.y="ADOS1")
?merge

for (row in 1:nrow(ADS)) {
  if (ADS[row,"VISIT"] == 1) {
  rep <-ADS[row,"ADOS"]}
  ADS[row,"ADOS"]<-rep
}
