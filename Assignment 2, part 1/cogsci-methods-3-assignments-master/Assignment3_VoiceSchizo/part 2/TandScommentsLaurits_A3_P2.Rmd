...
#making list with z-scored feature names
...
for (f in folds) {
  ...
  # Creating the full dataframe of predictions (can also be done like this: predictions[data$Subject %in% f] = test$predictions, need to create emtpy vector in before loop, like this predictions = rep(NA, nrow(data)))
  if (!is.null(testSet)){
...
for (f in folds) {
  ...
  for (zf in zfeatures) {
    ...
    eval(parse(text=paste('test$pred',zf,'[test$pred',zf,' > 0.5] = "Schizo"',  sep='')))
...
for (zf in zfeatures){
  ...
  print(paste(zf,"has an accuracy of:",round(matrix$overall[1],3)))
}
