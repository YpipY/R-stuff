...
```{r}
setwd("/Users/signeklovekjaer/Documents/Cognitive Science/3. semester/Experimental methods 3/1. class/Assignment 2, part 1/cogsci-methods-3-assignments-master/Assignment3_VoiceSchizo")
...
#setwd("/Users/signeklovekjaer/Documents/Cognitive Science/3. semester/Experimental methods 3/1. class/Assignment 2, part 1/cogsci-methods-3-assignments-master/Assignment3_VoiceSchizo/Pitch/")

path = "/Users/signeklovekjaer/Documents/Cognitive Science/3. semester/Experimental methods 3/1. class/Assignment 2, part 1/cogsci-methods-3-assignments-master/Assignment3_VoiceSchizo/Pitch/"
...
folds <- createFolds(unique(all_pitch$Sub_Diag), k=5, list = TRUE, returnTrain = FALSE) #create folds. Unique() puts each unique kid in a fold, so all data from one subj goes into one fold
...
for (f in folds) {
  ...
  temp_test = filter(all_pitch, Sub_Diag %in% f) #could also have used train_data[f,]
  temp_train = filter(all_pitch, ! Sub_Diag %in% f) #could also have used train_data[!f,]
  
  #run models on train
  mean_model <- lmer(mean_f0 ~ Diagnosis.x + Gender + (1| Sub_Diag) + (1| triangles), all_pitch, REML = F)
  
  #predict from the two models on both train and test data, train data does not really make sense --> just pedagogic 
  mean_predict <- predict(mean_model, temp_test, allow.new.levels = TRUE)
  ...
  rmse_mean <- c(rmse_mean, rmse(temp_test$mean, mean_predict))
