...
# Loading packages
library(pacman)
p_load(stringr,tidyverse,plyr,lmerTest,caret,Metrics,dplyr)

setwd("C:/Users/slmoni/Documents/Uni/Experimental Methods III/R-stuff/Assignment3")
```
...
data <- data.frame(soundname=character(),
                Study=numeric(),
                Subject=numeric(),
                 Diagnosis=numeric(),
                Triangles=numeric(),
...
for (i in files){
  ...
  data[n,6] <- mean(temp2$f0)
  data[n,7] <- sd(temp2$f0)
  ran<-range(temp2$f0)
  data[n,8] <- ran[2]-ran[1]
  data[n,9] <- median(temp2$f0)
  data[n,10] <- quantile(temp2$f0, probs=0.05)
  data[n,11] <- quantile(temp2$f0, probs=0.25)
  data[n,12] <- quantile(temp2$f0, probs=0.75)
  data[n,13] <- quantile(temp2$f0, probs=0.95)
}
...
table(pitchData$Diagnosis)

# Setting the right name for soundname
pitchData$soundname<-str_remove(pitchData$soundname,"_f0.txt")
